...
# Öğrenci Verimport streamlit as st
import pandas as pd
import os
...
# Sol Panel: Projeler ve Menü
st.sidebar.title("📁 Projelerim")
project_path = "./projects"  # Proje dosyalarının bulunduğu klasör
if os.path.exists(project_path):
    project_files = [f for f in os.listdir(project_path) if f.endswith(".py")]
    selected_project = st.sidebar.selectbox("Diğer Projelerim", project_files)
    if selected_project:
        st.sidebar.write(f"Seçilen Proje: {selected_project}")
        st.sidebar.write("[Projeyi Görüntüle](./projects/" + selected_project + ")")
else:
    st.sidebar.warning("Proje klasörü bulunamadı!")
